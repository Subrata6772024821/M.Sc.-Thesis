"""
Combined Density Calibration -- all 3 methods x 3 proxy pairs
================================================================

Builds ONE output workbook with 9 sheets:
...
by hand, and how the GPR sheet was built in the previous script:
  ...
  M2  2nd-degree polynomial: Ideal ~ a0 + a1*Proxy + a2*Proxy^2
                             ...
                             justified or just fitting noise (for this
                             dataset's raw, non-DTW proxies, it generally
                             is NOT significant -- see the "Fit
                             diagnostics" block on each 2nd-deg sheet).
  M3  GPR:                   RBF + WhiteKernel, hyperparameters fit once
                             ...
                             natively in-sheet (see the earlier GPR-only
                             script for the full derivation/validation).

NOTE ON M2 COEFFICIENTS: the OLD "2nd deg Acc-Flow" sheet in your
Without_DTW workbook had hardcoded coefficients (58.68871, 1.164757,
-0.002672) that do NOT reproduce from a fresh least-squares fit on the
Sheet1 data in either of your uploaded files (the Occ-pair coefficients
DO reproduce exactly, so the fitting method itself -- numpy.polyfit
degree 2 -- is confirmed correct; the old Flow numbers look like a
stale/mismatched leftover). This script always fits fresh from
whatever Sheet1 data is in FILE_PATH, so it will self-correct.

Run in PyCharm:
    pip install openpyxl pandas numpy scikit-learn
"""
...
# CONFIG
# ---------------------------------------------------------------------------
FILE_PATH = r"D:\Thesis_Final Data Analysis\05_12_25_1315_1345\120_sec\05_12_25_1315_1345_Combined Density_120sec.xlsx"
SOURCE_SHEET = "Sheet1"

IDEAL_COL = "Density_Accumulation (vehicle/km/lane)"
PROXY_COLS = {
    "Flow_SMS": "Density_Flow_SMS (vehicle/km/lane)",
    "Flow_Snapshot": "Density_Flow_SNAPSHOT (vehicle/km/lane)",
    "Occ": "Density_Occupancy (vehicle/km/lane)",
}
...
OUTPUT_PATH = os.path.join(_parent_dir, f"{_base_name}_AllMethods.xlsx")
...
def fit_poly2_with_diagnostics(proxy, ideal):
    ...
    adding the x^2 term. (See the conversation for why this matters: a
    small/insignificant quadratic term means the curve is barely
    distinguishable from a straight line for this data.)"""
...
def write_poly2_sheet(wb, sheet_name, time_labels, time_s, ideal, proxy,
    ...
    # ---- Fit-quality diagnostics block (added per discussion: is the
    # quadratic term statistically justified, or just noise?) ------------
    diag_row = avg_row + 3
