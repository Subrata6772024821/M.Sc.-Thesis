...
alignment, across the 3 density proxies (Flow_SMS, Flow_Snapshot, Occupancy)
and the 3 calibration methods (Mean Multiplier, 2nd-Degree Poly, GPR) --
plus a "Before Calibration" baseline bar (raw proxy vs. reference, no
calibration applied at all) in every group.

Reads two workbooks:
  - the "Combined Density ... AllMethods.xlsx" file (no DTW)
  - the "AllMethods With DTW ...xlsx" file (DTW-aligned)

and produces two grouped bar charts (MAD, MPD), each with 6 x-axis groups
(3 proxies x 2 conditions: "No DTW" / "With DTW"), 4 bars per group
