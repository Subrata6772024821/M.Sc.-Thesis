...
for th in thresholds:
    print(f"\n--- Results for Confidence Threshold: {th} ---")
    df = pd.DataFrame(data[th], columns=['Time (s)', 'Car', 'Motorcycle', 'Bus', 'Total'])
    if df.empty:
        print("No vehicles detected at this threshold.")
    else:
        # Using to_string() to ensure the entire DataFrame is printed
